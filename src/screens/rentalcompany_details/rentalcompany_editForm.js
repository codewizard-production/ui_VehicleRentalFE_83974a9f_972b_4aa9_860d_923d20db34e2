...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], rentalCompanyId, rentalcompany;

        rentalcompany = row['rentalcompany'];
        rentalCompanyId = row['rentalcompany'].find((x) => x.type === 'keyid').value;
            ...
            if (changes.length > 0) {
                ...
                if (tmp.length > 0) {
                    ...
                    if (rslt.status) {
                        ...
                        if (Helper.IsNullValue(mapItem.navpropname)) rentalCompanyId = rslt.id;
        ...
        global.AlertPopup("success", "RentalCompany is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
