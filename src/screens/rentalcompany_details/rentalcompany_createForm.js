...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, rentalCompanyId;
        const mapItems = MapItems;

        let rentalcompany = row['rentalcompany'];


        // Add Or Update RentalCompany
        rslt = await Support.AddOrUpdateRentalCompany(rentalcompany, dropDownOptions, []);
        if (rslt.status) {
            rentalCompanyId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the RentalCompany and navigation entity if it is deos not exist
            let navItem = rentalcompany.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "CompanyID", value: parseInt(rentalCompanyId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateRentalCompany(data, dropDownOptions);
        ...
        global.AlertPopup("success", "RentalCompany is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
