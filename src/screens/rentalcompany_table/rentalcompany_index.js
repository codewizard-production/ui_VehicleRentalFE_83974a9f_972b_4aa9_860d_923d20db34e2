...
const columns = [
    { headerName: "CompanyID", field: "CompanyID", flex: 1 },
    { headerName: "CompanyName", field: "CompanyName", flex: 1 },
    { headerName: "CompanyRegistrationNo", field: "CompanyRegistrationNo", flex: 1 },
];
...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetRentalCompaniesCount(query)
            .then(async (res) => {
                if (res.status) {
        ...
        await Api.GetRentalCompaniesMulti(query, "")
            .then(async (res) => {
                if (res.status) {
                    _rows = res.values || [];
                    if (_rows.length > 0) {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetRentalCompanySingle({ CompanyID: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/RentalCompanies/edit/${id}`;
        if (type === 'view') _route = `/RentalCompanies/view/${id}`;
    ...
    return (
        ...
        <>
            <Container {...props}>
                <Box style={{ width: '100%', paddingBottom: 5 }}>
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                                ...
                                onClick={() => NavigateTo("/RentalCompanies/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
                ...
                <Box style={{ width: '100%' }}>
                    <DataTable keyId={'CompanyID'} columns={columns} rowsCount={rowsCount} rows={rows} sortBy={sortBy} pageInfo={pageInfo} onActionClicked={OnActionClicked}
